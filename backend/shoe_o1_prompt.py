...
def shoe_o1_video_prompt(job, *, creator_profile: str = "Female", reference_count: int | None = None) -> str:
    """Compact Kling O1 prompt kept below the 1700-character API limit."""
    ...
    prompt = _clean(f"""
        ...
        CUT · SHOT 2 · 0:03–0:07: New angle in the same car. Active hand-held rotation or on-foot angle, whichever best suits the shoe. Clearly show front-to-side profile and upper shape with stylish, realistic motion.
        CUT · SHOT 3 · 0:07–0:10: Close detail/hero finish. Reveal only a real visible feature—sole/tread, heel, stitching, tongue, laces, zipper, lining or texture—then finish on a strong three-quarter hero angle.
    """)
    if len(prompt) > 1700:
        raise RuntimeError(f"Kling O1 shoe prompt exceeded API limit ({len(prompt)} chars).")
    return prompt
