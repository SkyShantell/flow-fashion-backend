...
def shoe_o1_video_prompt(job, *, creator_profile: str = "Female", reference_count: int | None = None) -> str:
    ...
    prompt = _clean(f"""
        9:16 vertical, 5 seconds. Use the supplied approved Flow start image @image1 as the exact first frame. Begin by perfectly matching @image1, then move. Ignore the black placeholder @video1 visually. {extra_rule}
